...
// aspects and a status section to represent the status of the
// node.
#CiliumNode: {
...
#CiliumNodeSpec: {
	...
	"alibaba-cloud"?: {
		...
		// which security groups to use for the ENI.
		"security-group-tags"?: {
			[string]: string
		}

		// SecurityGroups is the list of security groups to attach to any
		// ENI that is created and attached to the instance.
	...
	eni?: {
		...
		// the associated instance is terminated. If the parameter is not
		// set the default behavior is to delete the ENI on instance
		// termination.
		...
		// ENIs for Cilium IP allocation. Any interface matching this set
		// of tags will not be managed by Cilium.
		...
		// be used for IP allocation, eth0 will be ignored for PodIP
		// allocation.
		"first-interface-index"?: >=0 & int

		// InstanceID is the AWS InstanceId of the node. The InstanceID is
		// used to retrieve AWS metadata for the node.
		...
		// watermark. Going above the watermark can help reduce the
		// number of API calls to allocate IPs, e.g. when a new ENI is
		// allocated, as many secondary IPs as possible are allocated.
		...
		// when the node is first bootstrapped. It defines the minimum
		// base socket of addresses that must be available. After
		// reaching this watermark, the PreAllocate and MaxAboveWatermark
		...
		// brought up with. It is used as a sensible default subnet to
		// create ENIs in.
		"node-subnet-id"?: string

		// PreAllocate defines the number of IP addresses that must be
		// available for allocation in the IPAMspec. It defines the
		// buffer of addresses available immediately without requiring
		...
		// what AWS security groups to use for the ENI.
		"security-group-tags"?: {
			[string]: string
		}
		...
		// AWS subnets to use for ENI and IP allocation.
		"subnet-ids"?: [...string]
	...
	// from the node name which is typically the FQDN of the node.
	// The InstanceID typically refers to the identifier used by the
	// cloud provider or some other means of identification.
	"instance-id"?: string

	// IPAM is the address management specification. This section can
	// be populated by a user or it can be automatically populated by
	// an IPAM operator.
	ipam?: {
		// IPv6Pool is the list of IPv6 addresses available to the node
		// for allocation. When an IPv6 address is used, it will remain
		// on this list but will be added to Status.IPAM.IPv6Used
		"ipv6-pool"?: {
			[string]: {
				// Owner is the owner of the IP. This field is set if the IP has
				// been allocated. It will be set to the pod name or another
				// identifier representing the usage of the IP
				...
				// represents what resource the IP is associated with, e.g. in
				// combination with AWS ENI, this will refer to the ID of the ENI
				resource?: string
			}
		}
		...
		// to the node. When the current amount of allocated IPs will
		// approach this value, the considered value for PreAllocate will
		// decrease down to 0 in order to not attempt to allocate more
		// addresses than defined.
		"max-allocate"?: >=0 & int
		...
		// allocation. When an IP is used, the IP will be added to
		// Status.IPAM.Used
		podCIDRs?: [...string]

		// Pool is the list of IPv4 addresses available to the node for
		// allocation. When an IPv4 address is used, it will remain on
		// this list but will be added to Status.IPAM.Used
		pool?: {
			[string]: {
				// Owner is the owner of the IP. This field is set if the IP has
				// been allocated. It will be set to the pod name or another
		...
		pools?: {
			...
			// node. The operator will add new pod CIDRs to this field,
			// whereas the agent will remove CIDRs it has released.
			allocated?: [...{
			...
			// how many addresses this node requests out of each pool listed
			// here. This field is owned and written to by cilium-agent and
			// read by the operator.
			requested?: [...{
				// Needed indicates how many IPs out of the above Pool this node
				// requests from the operator. The operator runs a reconciliation
				// loop to ensure each node always has enough PodCIDRs allocated
				// in each pool to fulfill the requested number of IPs here.
				needed?: {
